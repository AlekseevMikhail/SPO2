...
static void fill_types(const char * filename, struct flow_graph_expr * expr, struct ast_analyze_error_list * errors) {
    ...
    switch (expr->_type) {
        case FLOW_GRAPH_EXPR_TYPE_BINARY:
            ...
            const bool eq = ast_type_reference_equals(expr->binary.lhs->type, expr->binary.rhs->type);
            ...
            switch (expr->binary.op) {
                ...
                case AST_EXPR_BINARY_OP_GE:
                    if (!eq && !num) {
                        raise_error("types of operands must be same", filename, expr->position, errors);
                    } else if (ast_type_reference_is_custom(expr->binary.lhs->type)) {
                        raise_error("comparison of custom types is not supported", filename, expr->position, errors);
                    }
...
static void check_return_types(
    ...
    switch (node->_type) {
        case FLOW_GRAPH_NODE_TYPE_EXPR:
            ...
            if (!ast_type_reference_is_subtype(node->expr.expr->type, subroutine->return_type)) {
                raise_error(
    ...
    *errors = ast_analyze_error_list_init();
    ...
    for (size_t i = 0; i < subroutines->size; ++i) {
        ...
        for (size_t j = 0; j < subroutine->nodes.size; ++j) {
            ...
            struct flow_graph_expr * expr = NULL;
            ...
            fill_types(subroutine->filename, expr, errors);
        }
    }
